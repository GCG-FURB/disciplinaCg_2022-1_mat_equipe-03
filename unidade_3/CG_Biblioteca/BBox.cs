  ...
  public class BBox
  {
    ...
    public bool EstaDentro(Ponto4D pto)
     { 
      ...
      bool retorno = false;
      if( (pto.X <= this.maiorX && pto.X >= this.menorX) && (pto.Y <= this.maiorY && pto.Y >= this.menorY)){
        retorno=true;
...
//(pto.X >= bBox.obterMenorX )&& (pto.X <= bBox.obterMaiorX )
//&& (pto.Y >= bBox.obterMenorY && pto.Y <= bBox.obterMaiorY) && pto.Z >= bBox.obterMenorZ && pto.Z <= bBox.obterMaiorZ;
